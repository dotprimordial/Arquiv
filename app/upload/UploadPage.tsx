  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await processAndUploadNorm({
        ...
        fileType: contentType === 'docx' ? 'pdf' : contentType,
                ...
                <div className="space-y-2">
                  ...
                  <div className="rich-text-editor">
                    <ReactQuill
                      ...
                      modules={modules}
                      formats={formats}
                      placeholder="Cole ou digite aqui o texto completo da norma com formatacao..."
                      className="bg-zinc-50 rounded-3xl overflow-hidden border border-zinc-100"
                    />
                  </div>
                  <div className="flex justify-between items-center pt-2">
                    <div className="text-xs text-zinc-400 font-medium">
                      {normContent.replace(/<(.|\n)*?>/g, '').length.toLocaleString()} caracteres (sem formatacao)
                    </div>
                    {isAnalyzing && (
