...
export default function NormDetailPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      setError(null);
      try {
        // FIX: getFullNormContent agora trata tudo internamente:
        // 1º tenta Supabase, 2º usa IA como fallback
        const normContent = await getFullNormContent(country, code);
        
        // Verificar se é um PDF
        if (normContent.startsWith('PDF:')) {
          const url = normContent.substring(4); // Remove 'PDF:' prefix
          setIsPdf(true);
          setPdfUrl(url);
          setContent(null);
        } else {
        ...
        // FIX: detectar se é oficial ou gerado por IA
        // getFullNormContent retorna conteúdo real se no Supabase, ou IA se não estiver
        // setContent(normContent);
        
        // Tentar confirmar se é oficial ou gerado por IA
        // (getFullNormContent retorna conteúdo real se no Supabase, ou IA se não estiver)
        setIsAIGenerated(false); // será atualizado abaixo
      } catch (err: unknown) {
          ...
          >
            <div className="p-8 md:p-12 border-b border-zinc-100 bg-zinc-50/50">
              ...
              <p className="text-zinc-500 text-lg leading-relaxed">
                Detalhes técnicos e requisitos regulamentares completos.
              </p>
            </div>
