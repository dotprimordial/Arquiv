...
const RippleButton: React.FC<RippleButtonProps> = ({
  ...
  if (variant === 'hover') {
    ...
    return (
      <>
        ...
        <button className={hoverButtonFinalClassName} onClick={handleButtonClick} disabled={disabled} type={type} title={title}>
  ...
  if (variant === 'ghost') {
    const ghostButtonFinalClassName = [
      "relative", "border-none", "isolate", "overflow-hidden", "cursor-pointer",
      disabled ? "opacity-50 cursor-not-allowed pointer-events-none" : "",
      className,
    ].filter(Boolean).join(" ");
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <button className={ghostButtonFinalClassName} onClick={handleButtonClick} disabled={disabled} type={type} title={title}>
          {jsRippleElements}
          {children}
        </button>
      </>
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
      <button className={buttonClasses} onClick={handleButtonClick} disabled={disabled} type={type} title={title}>
